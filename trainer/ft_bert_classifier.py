...
class BertFinetunePipeline(BasePipeline):
    """
    BERT分类器的训练pipeline类
    """
    ...
    def _init_model(self):
        ...
        model_config = BertClassifierConfig(
            model_name=self.config_manager.get('model_name', 'bert-base-uncased'),
    ...
    def _init_dataset(self):
        ...
        """
        # 确保数据集类型为text
        if self.config_manager.get('dataset_type', 'text') != 'text':
            raise ValueError("BERT Pipeline only supports text dataset type")
